...
- `validateDataset(dataset, shapesDataset, overrideReportURI, overrideReportUuid) => Promise (Report)`: Function for validating a RDF/JS dataset with an RDF/JS dataset containing SHACL shapes. Note: the original validation report is enriched to support UUIDs, no blank nodes etc. A promise holding a Report object is returned.   
